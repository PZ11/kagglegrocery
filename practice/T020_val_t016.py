...
def prepare_dataset(t2017, is_train=True):
    X = pd.DataFrame({
        ...
        "mean_ly_wk3_2017": get_timespan(df_2017, t2017, 364, 21).mean(axis=1).values,
...
test_e.to_pickle('../data/T020_val_t016.p')
...
#------------------------------------------------------------------------------------------#
#Merge with zero on discontinue SKUs
df_prev = submission

df_subzero= pd.read_csv('../input/sub_zero3m.csv')

t_new = pd.merge(df_prev, df_subzero,  on=['id'], how = 'left')
...
submission.to_csv('../submit/T020_mg_ly_wk123_dow.csv', float_format='%.4f', index=None, compression='gzip')
"""
